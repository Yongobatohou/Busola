...
import os
from dashadmin.models import Article, CategorieArticle, Projet, Album, Galerie, Ressource, Benevole, User, Partenariat, Member
...
def team_list(request):
    membres = Member.objects.all().order_by('-date_creation')
    return render(request, 'equipe/equipe-list.html', {'membres': membres})
...
def team_add(request):
    ...
    return render(request, 'equipe/equipe-list.html')

def team_edit(request, id):
    membres = Member.objects.get(id=id)
    return render(request, 'equipe/equipe-edit.html', {'membre': membres})

def team_update(request, id):
    membre = Member.objects.get(id=id)

    if request.method == 'POST':
        membre.nom = request.POST.get('nom')
        membre.role = request.POST.get('role')
        membre.email = request.POST.get('email')
        membre.contact = request.POST.get('contact')
        membre.facebook = request.POST.get('facebook')
        membre.linkedin = request.POST.get('linkedin')
        membre.twitter = request.POST.get('twitter')
        membre.description = request.POST.get('description')
    ...
    return render(request, 'equipe/equipe-edit.html', {
        'membre': membre
    })
...
def team_delete(request, id):
    ...
    if membre.photo and membre.photo.path:
        if os.path.isfile(membre.photo.path):
            os.remove(membre.photo.path)
    membre.delete()
    return redirect('team_list')
